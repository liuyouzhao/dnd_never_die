/*********
DotDot类[实例化类]：
1.一些技能可以造成组合属性削弱效果，例如移动速度和攻击速度降低等，这些组合效果被称为一个DeDot
2.一些技能可以为友方提供组合增强效果，例如移动速度和攻击力增加，这些组合效果被称为一个Dot
...
/// Class Dot/DeDot
Dot = Klass({
	initialize: function(dat, time_run, time_dot, owner, unit, target_name){
		/// nothing to do
		this.data = dat;
	...
	clearChanged: function(){
		...
		//[TODO]这里其实只是为了清空一个哈希表，为了保持原来哈希的可能的应用关系，建议这样子清空
		/*
